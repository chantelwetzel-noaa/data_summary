...
format_hkl_data <- function(
  ...
  data$ave_lat <- data$ave_long <- NA 
  for (aa in unique(data$site_number)) {
    find <-  which(data$site_number == aa) 
    data$ave_long[find] <-  -1 * mean(data[find, "drop_longitude_degrees"])
    data$ave_lat[find] <-  mean(data[find, "drop_latitude_degrees"])
  }
